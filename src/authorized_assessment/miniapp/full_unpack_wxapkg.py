...
def main():
    parser = argparse.ArgumentParser(description="解密并解析微信小程序 wxapkg 包")
    parser.add_argument("appid", help="小程序 AppID")
    parser.add_argument("pkg_path", type=Path, help="wxapkg 文件路径")
    parser.add_argument("out_dir", type=Path, help="源码输出目录")
    args = parser.parse_args()
    appid, pkg_path, out_dir = args.appid, args.pkg_path, args.out_dir

    if not pkg_path.is_file():
        parser.error(f"wxapkg 文件不存在: {pkg_path}")
