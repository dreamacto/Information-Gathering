...
## 开工前必读
- knowledge_base/ 属施工中机制（W 施工项），当前**没有消费者脚本**。若目录不存在就声明"knowledge_base 未落地"，本轮只做聚合与报告，不建目录、不写未落地文件。
- metrics_weekly.py（W10）尚不存在时跳过并记录原因，不阻塞沉淀。
...
4. 跑 metrics_weekly.py（W10 提供；尚不存在则跳过并记录原因）产出本周指标；不阻塞沉淀。
5. 零网络请求：只用盘上已有数据。
...
- 输出文件：
  ...
  - 无论知识库是否落地：`reports/weekly_YYYY-MM-DD.md`（本周汇总 + 下周建议；知识库未落地时把指纹/误报/命中统计写入本报告，落盘路径以 reports/ 为准）
