...
- 知识库注意：`knowledge_base/` 属施工中机制（W 施工项），当前**没有消费者脚本**。若目录不存在就**不要创建**，把假设写回 run 工作区（见输出契约），并注明"knowledge_base 未落地"。
...
## 输出契约
- 落盘位置：`<被分析 run 目录>/postrun_review/hypothesis_plan.jsonl`（本 run 工作区内，UTF-8；目录不存在则先创建）——因为 knowledge_base 未落地，假设不得写全局知识库，只落当前 run。
- schema（一个假设一行）：
```json
{
  "hypothesis": "一句话假设",
  ...
  "priority": "P0|P1|P2",
  "note": "knowledge_base 未落地，暂落 run 工作区"
}
```
