...
// Output TXT for this project runner: butian_targets_for_runner_*.txt
// Format: one target per line, "https://host|靶标名称".
...
(async () => {
  ...
  function buildRunnerTargetText(data) {
    const seen = new Set();
    const lines = [
      "# Butian targets for gov_exercise_runner.py / one_click_workflow.py",
      "# Format: URL|name",
    ...
    return { text: "\ufeff" + lines.join("\n"), count: seen.size, skipped };
  }

  function downloadRunnerTargets(data, suffix = "") {
    const built = buildRunnerTargetText(data);
    const blob = new Blob([built.text], { type: "text/plain;charset=utf-8" });
    const a = document.createElement("a");
    const stamp = new Date().toISOString().replace(/[:.]/g, "-").slice(0, 19);
    const filename = `butian_targets_for_runner_${suffix ? suffix + "_" : ""}${stamp}.txt`;
    ...
    a.remove();
    console.log(`[butian-iframe-domain] exported ${built.count} runner targets, skipped ${built.skipped} invalid rows -> ${filename}`);
  }

  const cached = existingResults();
  if (cached.length) {
    const choice = prompt(`发现 iframe 缓存 ${cached.length} 条：1=直接导出，2=继续/补跑，0=清空重来，9=停止当前任务`, "1");
    if (choice === "1") return downloadRunnerTargets(cached, "cached");
    if (choice === "9") {
  ...
  for (const row of rowsToRun) {
    ...
    if (shouldStopAfterSave) {
      downloadRunnerTargets(saveResults(results), "geetest_pause");
  ...
  if (!finalDownloaded) downloadRunnerTargets(saveResults(results), "captured");
})();
