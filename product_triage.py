...
RULES: tuple[ProductRule, ...] = (
    ...
    ProductRule("springboot", "Spring Boot", "middleware", (r"spring.?boot", r"spring_actuator", r"/actuator", r"actuator_"), ("springboot", "spring-boot", "actuator"), ("runner high-value paths", "dddd/nuclei"), ("manual browser",), "java_springboot", "Only read actuator metadata by default; heapdump/env needs careful review."),
    ...
    ProductRule("fastjson", "Fastjson", "java_component", (r"fastjson", r"com\.alibaba\.fastjson"), ("fastjson"), ("dddd/nuclei", "afrog"), ("manual request review",), "java_fastjson", "Treat RCE templates as approval-gated; version detection is safe to queue."),
    ProductRule("weblogic", "WebLogic", "middleware", (r"weblogic", r"bea weblogic", r"/console/login"), ("weblogic"), ("dddd/nuclei", "afrog"), ("manual browser",), "middleware_weblogic", "Prefer console/version/protocol detection; exploit checks need approval."),
    ProductRule("struts2", "Apache Struts", "framework", (r"struts", r"struts2", r"\.action\b"), ("struts"), ("dddd/nuclei", "afrog"), ("manual browser",), "java_struts", "Only route candidates; OGNL/RCE payload validation is approval-gated."),
    ProductRule("tomcat", "Apache Tomcat", "middleware", (r"tomcat", r"catalina", r"/manager/html", r"apache-coyote"), ("tomcat"), ("runner high-value paths", "dddd/nuclei"), ("manual browser",), "middleware_tomcat", "Manager/default-login checks must not brute force."),
    ProductRule("nacos", "Nacos", "middleware", (r"\bnacos\b", r"/nacos/"), ("nacos"), ("dddd/nuclei", "manual browser"), ("afrog",), "middleware_nacos", "Default identity/token checks are sensitive; queue for single-target review."),
